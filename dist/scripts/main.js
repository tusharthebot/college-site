...
function truncate_lines(target, lines, after, ellipsis) {
  ...
  if ($target.length) {
    ...
    if (typeof after === 'undefined') after = '';else lines = lines + 1;
    ...
    if (object_exists(window, 'jQuery') && object_exists(jQuery, 'fn', 'dotdotdot')) {
      $target.dotdotdot({
        ellipsis: ellipsis,
        height: targetHeight,
        watch: true,
        after: after,
        callback: function callback(isTruncated, origContent) {
          if (!isTruncated) {
      ...
      var e,
        ...
        for (x = "", this.maxRows = 4, C = 0; U[1] > C; C++) {
          if (D = this._daylightSavingAdjust(new Date(te, Z, t.selectedDay)), I = " ui-corner-all", P = "", $) {
            if (P += "<div class='ui-datepicker-group", U[1] > 1) switch (C) {case 0:
                P += " ui-datepicker-group-first", I = " ui-corner-" + (B ? "right" : "left");
                break;case U[1] - 1:
                P += " ui-datepicker-group-last", I = " ui-corner-" + (B ? "left" : "right");break;default:
                P += " ui-datepicker-group-middle", I = "";}P += "'>";
          }for (P += "<div class='ui-datepicker-header ui-widget-header ui-helper-clearfix" + I + "'>" + (/all|left/.test(I) && 0 === k ? B ? o : s : "") + (/all|right/.test(I) && 0 === k ? B ? s : o : "") + this._generateMonthYearHeader(t, Z, te, Q, J, k > 0 || C > 0, f, g) + "</div><table class='ui-datepicker-calendar'><thead>" + "<tr>", M = u ? "<th class='ui-datepicker-week-col'>" + this._get(t, "weekHeader") + "</th>" : "", w = 0; 7 > w; w++) {
            T = (w + c) % 7, M += "<th scope='col'" + ((w + c + 6) % 7 >= 5 ? " class='ui-datepicker-week-end'" : "") + ">" + "<span title='" + d[T] + "'>" + p[T] + "</span></th>";
          }for (P += M + "</tr></thead><tbody>", S = this._getDaysInMonth(te, Z), te === t.selectedYear && Z === t.selectedMonth && (t.selectedDay = Math.min(t.selectedDay, S)), H = (this._getFirstDayOfMonth(te, Z) - c + 7) % 7, z = Math.ceil((H + S) / 7), N = $ ? this.maxRows > z ? this.maxRows : z : z, this.maxRows = N, W = this._daylightSavingAdjust(new Date(te, Z, 1 - H)), A = 0; N > A; A++) {
            for (P += "<tr>", O = u ? "<td class='ui-datepicker-week-col'>" + this._get(t, "calculateWeek")(W) + "</td>" : "", w = 0; 7 > w; w++) {
              E = m ? m.apply(t.input ? t.input[0] : null, [W]) : [!0, ""], F = W.getMonth() !== Z, L = F && !v || !E[0] || Q && Q > W || J && W > J, O += "<td class='" + ((w + c + 6) % 7 >= 5 ? " ui-datepicker-week-end" : "") + (F ? " ui-datepicker-other-month" : "") + (W.getTime() === D.getTime() && Z === t.selectedMonth && t._keyEvent || b.getTime() === W.getTime() && b.getTime() === D.getTime() ? " " + this._dayOverClass : "") + (L ? " " + this._unselectableClass + " ui-state-disabled" : "") + (F && !_ ? "" : " " + E[1] + (W.getTime() === G.getTime() ? " " + this._currentClass : "") + (W.getTime() === Y.getTime() ? " ui-datepicker-today" : "")) + "'" + (F && !_ || !E[2] ? "" : " title='" + E[2].replace(/'/g, "&#39;") + "'") + (L ? "" : " data-handler='selectDay' data-event='click' data-month='" + W.getMonth() + "' data-year='" + W.getFullYear() + "'") + ">" + (F && !_ ? "&#xa0;" : L ? "<span class='ui-state-default'>" + W.getDate() + "</span>" : "<a class='ui-state-default" + (W.getTime() === Y.getTime() ? " ui-state-highlight" : "") + (W.getTime() === G.getTime() ? " ui-state-active" : "") + (F ? " ui-priority-secondary" : "") + "' href='#'>" + W.getDate() + "</a>") + "</td>", W.setDate(W.getDate() + 1), W = this._daylightSavingAdjust(W);
            }P += O + "</tr>";
...
(function ($) {
  $(window).on("load", function () {
    ...
    $(".video-overlay-product .content-outer .close-overlay").on("click", function () {
